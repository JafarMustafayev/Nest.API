...
public interface IReadRepository<T> : IRepository<T> where T : BaseEntity, new()
{
    ...
    public IQueryable<T?> GetAllByExpression(Expression<Func<T, bool>> expression, int page = 1, int take = 2, bool isTracking = true, Expression<Func<T, object>>? OrderBy = null, List<Expression<Func<T, object>>>? includes = null, List<(Expression<Func<T, object>> include, Expression<Func<object, object>> thenInclude)>? thenIncludes = null);
